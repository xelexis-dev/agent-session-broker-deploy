...
MCP 연결 후 클라이언트가 실제 현재 대화를 확인하면 토큰의 닉네임으로 **자동 등록**합니다. 별도의 등록 프롬프트는 필요 없습니다. 관리 패널의 **프로젝트 세션 관리 → 세션 토큰**에서 등록 대기가 해제되었는지, 세션 목록에 닉네임이 표시되는지 확인합니다. MCP 연결 성공만으로 서버 등록 성공을 뜻하지는 않습니다.
