...
시스템 관리자는 **사용자 관리**에서 계정을 생성하고 활성 상태·시스템 역할·비밀번호를 관리합니다. 프로젝트 생성자는 Owner이며 **프로젝트 설정**에서 여러 Owner, Developer, Viewer를 지정할 수 있습니다. Owner는 설정·멤버·전체 토큰과 세션을 관리하고, Developer는 본인의 토큰과 세션을 관리하며, Viewer는 조회만 합니다. 미배정 사용자는 프로젝트에 접근할 수 없고 마지막 활성 Owner는 제거할 수 없습니다.
...
사이드 메뉴의 **내 프로필**에서 표시 이름·이메일과 비밀번호를 변경합니다. 비밀번호 변경에는 현재 비밀번호와 새 비밀번호 확인이 필요하며, 변경 후 모든 기존 로그인이 해제됩니다. 로그인 아이디와 권한은 프로필에서 바꿀 수 없습니다.
