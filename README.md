...
Codex가 MCP 시작 시 대화 ID를 제공하지 않는 환경에서는 첫 일반 도구 호출의 대화 정보로 자동 등록한 뒤 그 요청을 처리합니다. 예를 들어 `x-broker에 연결된 세션을 보여줘`라는 일반 조회로 시작할 수 있습니다. 이 경우 최초 도구 호출 전까지는 등록 대기가 정상이며, 클라이언트가 임의의 대화를 골라 등록하지 않습니다. Claude의 구형 채널 방식은 자동 등록 이후 채널 활성화 응답이 필요할 수 있습니다. 활성화 전에는 등록된 상태여도 메시지 수신 준비가 완료되지 않습니다.

`--token`은 필수입니다. 값을 누락하거나 공백·개행을 넣으면 실패하며 무인증 연결로 전환하지 않습니다. 토큰 값을 바꿨다면 MCP 연결도 다시 시작합니다. 0.3.1 이하의 `--token-file` 설정은 `--token`과 실제 토큰 값으로 바꿉니다.
